...
download_images_flag = getenv("DOWNLOAD_IMAGES")
...
def download_images(url):
	...
	extract = "unzip -o " + archive + " -d " + base_path
        killxnbak = "sudo rm -rf " + base_path + "*.xnbak.jpg"
	system(remove)
	system(download)
	system(extract)
	system(killxnbak)
...
def resize_images():
	...
	for file in images:
		img = Image.open(file)
		if basewidth/baseheight < float(img.size[0])/float(img.size[1]):
